...
class DashboardSupabaseUser extends BaseAuthUser {
  DashboardSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dashboardSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DashboardSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
